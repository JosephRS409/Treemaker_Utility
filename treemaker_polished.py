...
# CHANGES DIRECTORY
def change_dir(): # The junction function between make_dir() or look_or_move().
    try:
        key = True
        while key == True:
            # try:
    ...
    try:
        ...
        while key == True:
            ...
            # Will this scope allow me to use "inquiry" variable again?
                # Yes, because this is outside the other "inquiry"'s scope.
                # Defining a variable in an if statement will work outside its scope.
            if inquiry == "yes":
                create_folder()
                key = False
                pass
            # IF YES, GO TO FILE MAKER "create_folder()"
                ...
                # Should I just let the loop break or call the main function?
                   # Don't call the main function. (Endless loop)
                   # Breaking this loop takes it to the function make_dir() is in. a.k.a. "function callback"
                pass
            else:
                pass
    except ValueError as ex:
    ...
    try:
        ...
        while key_two == True:
            ...
            for directory in os.listdir(parent_dir):
                if os.path.isdir(directory) == True:
                    print(directory)
            new_path = input("\nThese are the current folders in this directory.\n Please give your directory a new name: ")
            print()
